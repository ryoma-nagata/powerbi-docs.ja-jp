...
この設定は、データ更新とレポート表示に適用されます。 データ更新では、クエリの更新が無効にされていない限り、データソースからのデータの更新とクエリの更新の両方が実行されます。 クエリの更新が無効にされていない場合は、このメモリ制限はそれらのクエリにも適用されます。 失敗したクエリがあると、データ更新が成功した場合でも、スケジュールされた更新の状態はエラーとして報告されます。

#### <a name="query-timeout"></a>クエリのタイムアウト

この設定を使用すると、実行時間の長いクエリのより適切な制御が維持されますが、ユーザーのレポートの読み込みに時間がかかることがあります。 この設定は、データ更新とレポート表示に適用されます。 データ更新では、クエリの更新が無効にされていない限り、データソースからのデータの更新とクエリの更新の両方が実行されます。 クエリの更新が無効にされていない場合、このタイムアウト制限はそれらのクエリにも適用されます。
