...
- 両方のワークスペース (コンテンツが含まれるワークスペースと、データセットが含まれるワークスペース) が Premium 容量でホストされている場合、Pro ライセンスを持たないユーザーが使用できるのは、共有データセットで構築されたレポートとダッシュボードだけです。
