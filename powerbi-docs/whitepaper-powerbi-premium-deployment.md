...
**作成者:** [Peter Myers](https://www.linkedin.com/in/peterjsmyers)(データプラットフォーム MVP、およびビットごとのソリューションを持つ独立した BI エキスパート)

**技術校閲者:** Adam Saxton、Akshai Mirchandani、Bhavik マーチャント、David Magar、Josh Caplan、Michael Blythe である、Nimrod Shalit、Olivier Matrat、Swati Gupta
...
次の2つの方法で使用できます。クラウドサービスとして、および**Power BI Report Server**というオンプレミスのレポートソリューションとして機能します。 \[[1](#endnote-01)\]
...
**容量**は、Power BI コンテンツのホストと配信に使用される一連のリソース (ストレージ、プロセッサ、およびメモリ) を表す中核的な Power BI 概念です。 容量は、共有または専用です。 **共有容量**は他の Microsoft の顧客と共有されますが、**専用の容量**は1人の顧客に完全にコミットされます。 専用容量については、 [Premium 容量](#premium-capacities)に関するトピックを参照してください。

共有された容量では、他の顧客と共有されている計算リソースでワークロードが実行されます。 容量がリソースを共有する必要があるため、最大モデルサイズ (1 GB) と最大日次更新頻度 (1 日に8回) などの "公平な再生" を保証するための制限が課せられます。

### <a name="workspaces"></a>ワークスペース

Power BI のワークスペースは容量内に存在し、セキュリティ、コラボレーション、デプロイコンテナーを表します。 各 Power BI ユーザーには、**マイ ワークスペース**と呼ばれる個人用のワークスペースが用意されます。 追加のワークスペースを作成することで、コラボレーションや配置を有効にすることができます。これらは**アプリのワークスペース**と呼ばれます。 既定では、個人用ワークスペースを含むワークスペースが共有された容量で作成されます。
...
すべての Power BI コンテンツは、Power BI コンテンツ用のコンテナーであるワークスペース内で格納および管理されます。 各 Power BI ユーザーには個人用ワークスペースがありますが、一般的なベストプラクティスは、アプリワークスペースを作成することです。 アプリワークスペースを使用すると、コンテンツの共同所有権を有効にし、コンテンツに対して共同作業を行うことができます。 また、コンテンツをステージングして、世界中のユーザーにアプリとして配布する機能も提供します。
...
データフローは、アプリワークスペースでのみ作成および管理でき、Azure Data Lake Storage Gen2 の Common Data Model (CDM) にエンティティとして格納されます。 通常、最新のデータを保存するために定期的に更新するようにスケジュールされています。
...
データセット3は\[、ストリーミングデータセット[3](#endnote-03)\]を除き、Analysis Services の成熟したモデリングテクノロジを活用するデータモデルを表します。
...
Power BI ブックは、Power BI コンテンツタイプ\[ [4](#endnote-04)\]です。 これらは、Power BI サービスにアップロードされた Excel ブックであり、データセット (モデル) を作成する、アップロードした Excel ブックと混同しないようにしてください。 ブックのコンテンツタイプは、ブックへの接続を表します。これは、Power BI サービスにアップロードするか、OneDrive または SharePoint Online のクラウドストレージに残しておくことができます。
...
レポートには、次の2種類があります。レポートとページ分割されたレポートを Power BI します。
...
- 通常、フィルターとスライサーの相互作用には、モデルに対するクエリの実行が含まれます。 たとえば、スライサーの選択を変更すると、既定では、ページ\[ [5](#endnote-05)の各ビジュアルの再読み込みが必要になります。\]
...
SQL Server Reporting Services (SSRS) レポート (\*.rdl 形式) のパブリケーションと表示を可能にする、**ページ**分割されたレポート。 これらの名前が示すように、ページ分割されたレポートは、要件によって固定ページサイズへの印刷が必要になった場合や、完全に展開する必要があるデータの変数リストがある場合によく使用されます。 たとえば、(ビジュアル内をスクロールするのではなく) 複数ページレンダリング用にデザインされた請求書を印刷することができます。
...
更新すると、データは圧縮されて最適化され、VertiPaq ストレージエンジンによってディスクに格納されます。 ディスクからメモリに読み込まれた場合は、10倍の圧縮が表示される可能性があるため、10 GB のソースデータが約 1 GB のサイズに圧縮されることを想定するのが妥当です。 ディスク上のストレージサイズは、最大 20% 削減できます。 \[[4/6](#endnote-06)\]
...
- モデルは、サポートされる1つのデータソースに基づいている必要があります。したがって、データの統合はデータソースで既に達成されている必要があります。 サポートされているデータソースは、多くの一般的なデータストア\[ [7](#endnote-07)\]をサポートするリレーショナルシステムと分析システムです。
...
**Power BI Pro**ライセンスを使用すると、個人はアプリワークスペース内で作成して共同作業し、Power BI コンテンツを共有および配布できます。 オンプレミスのデータソースなど、データセットの更新を構成して、データを自動的に最新の状態に保つこともできます。 さらに、データへのアクセス方法と使用方法を監査および制御できます。 このライセンスは、ユーザーが Power BI Premium 専用容量に関連付けられていない限り、他のユーザーから共有コンテンツを受信するために必要です。
...
- 大きなデータセットサイズ\[のサポート[8](#endnote-08)\]
...
別の方法として、1つの SKU ファミリを持つ Azure Power BI Embedded サブスクリプションを購入することもできます。埋め込みと容量テスト専用**の sku (** A1-A6)。
...
サブスクリプションとライセンスに関するトピックに記載されているように、SKU ファミリは2つ Power BI Premium ます。EM と P。すべての Power BI Premium Sku は容量ノードとして使用でき、それぞれがプロセッサ、メモリ、およびストレージで構成されるリソースのセット量を表します。 リソースに加えて、各 SKU には、1秒あたりの DirectQuery (DQ) 接続とライブ接続 (LC) 接続の数、および並列モデル更新の数に関する操作上の制限があります。
...
インポートモデルは、クエリや更新ができるように、メモリに完全に読み込む必要があります。 この Power BI サービスは、使用可能なメモリを最大限に活用できるように高度なアルゴリズムを使用してメモリ使用量を管理し、容量を過剰にコミットすることができます。容量に多数のインポートモデル (Premium 容量あたり最大 100 TB) を格納することは可能ですが、ディスクストレージの組み合わせがサポートされているメモリを超過した場合 (およびクエリと更新に追加のメモリが必要な場合)、これらすべてをメモリに読み込むことはできません。同じ時刻です。

そのため、インポートモデルはに読み込まれ、使用量に応じてメモリから削除されます。 インポートモデルは、まだメモリ内ではなく、または更新 (バックグラウンド操作) されるときに、クエリ (対話操作) されるときに読み込まれます。

メモリからモデルを削除することを**削除と呼び**ます。これは、モデルのサイズによっては、Power BI がすばやく実行できる操作です。 容量にメモリ不足が発生していない場合、モデルはそのままメモリに読み込まれ、メモリ内に残ります。 \[[10ただし、](#endnote-10)モデルの読み込みに使用できるメモリが不足している場合、Power BI サービスは最初にメモリを解放する必要があります。\] これは、過去3分\[ [11](#endnote-11)\]日に使用されていないモデルを検索して、非アクティブになったモデルを検出することによってメモリを解放します。 削除できる非アクティブ モデルが存在しない場合、Power BI サービスではバックグラウンド操作用に読み込まれたモデルの削除が試みられます。 これには、AI ワークロードなどのバックグラウンドワークロードの削除が含まれる場合があります。 最後の手段として、失敗した\[試行[11](#endnote-11)\]が30秒経過した後で、対話型操作に失敗します。 この場合、レポートユーザーには、すぐに再実行するようにという提案でエラーが通知されます。
...
- 容量管理者
- 容量のサイズ
- データ常駐\[のリージョン[12](#endnote-12)\]

少なくとも1つの容量管理者が割り当てられている必要があります。 容量管理者として割り当てられたユーザーは次のことが可能です
...
- 容量を再起動して、システムオーバーロード\[ [13](#endnote-13)の場合、すべての操作をリセットします。\]

容量管理者は、ワークスペースのアクセス許可が明示的に割り当てられていない場合はワークスペースのコンテンツにアクセスできず、利用状況の指標、監査ログ、またはテナントの設定など、Power BI のすべての管理領域にアクセスすることはできません。 重要なのは、容量管理者に新しい容量を作成したり、既存の容量をスケールしたりするためのアクセス許可がないことです。 また、容量ごとに割り当てられ、割り当てられている容量だけを表示および管理できるようにします。

容量のサイズは、プール内の使用可能な仮想コアの数によって制限されている、使用可能な SKU オプションの一覧から選択する必要があります。 プールから複数の容量を作成して、購入した1つ以上の Sku からソースを作成することができます。 たとえば、P3 SKU (32 v コア) を使用すると、1つの P2 (16 v コア) と2つの P1 (2 x 8 v コア) という3つの容量を作成できます。 パフォーマンスとスケールの向上は、サイズの小さい容量を作成することによって実現できます。このトピックについては、「 [Premium 容量の最適化](#optimizing-premium-capacities)」セクションを参照してください。 次の図は、5つの Premium 容量 (3 x P1、2 x P3) で構成される架空の Contoso 組織のセットアップ例を示しています。それぞれのアプリワークスペースと、共有容量の複数のワークスペースが含まれています。

![架空の Contoso 組織のセットアップ例](media/whitepaper-premium-deployment/contoso-organization-example.png)

Premium 容量は、Power BI テナントのホームリージョン以外のリージョンに割り当てることができます。これにより、コンテンツが存在する Power BI データセンター (定義された地域内) を管理者が制御できるようになります。 \[[vdc](#endnote-12)\]

Power BI サービス管理者と Office 365 のグローバル管理者は、Premium 容量を変更できます。 具体的には、次のことが可能です。
...
既定では、Premium 容量は Power BI クエリの実行に関連するワークロードをサポートします。 また、3つの追加のワークロードもサポートしています。**ページ**分割されたレポート、**データフロー**、 **AI**。 各ワークロードでは、ワークロードで使用できる最大メモリ (使用可能なメモリの合計に対する割合) を構成する必要があります。 最大メモリ割り当てを増やすことは、ホストできるアクティブなモデルの数と更新のスループットに影響を与える可能性があることを理解しておくことが重要です。

メモリはデータフローに動的に割り当てられていますが、ページ分割されたレポートには静的に割り当てられています。 最大メモリを静的に割り当てる理由は、ページ分割されたレポートは容量のセキュリティで保護された領域内で実行されるためです。 ページ分割されたレポートのメモリを設定するときは、モデルの読み込みに使用できるメモリが減るため、注意が必要です。

|                     | EM3                      | P1                       | P2                      | P3                       |
|---------------------|--------------------------|--------------------------|-------------------------|--------------------------|
| ページ分割されたレポート | N/A | 20% (既定値)、10% (最小値) | 20% (既定値)、5% (最小値) | 20% (既定値)、2.5% (最小値) |
| データフロー | 20% (既定値)、8% (最小値)  | 20% (既定値)、4% (最小値)  | 20% (既定値)、2% (最小値) | 20% (既定値)、1% (最小値)  |
| AI | N/A | 既定値は 20% です。最小 20%  | 20% (既定値)、10% (最小値) | 20% (既定値)、5% (最小値)  |
| | | | | |
...
ワークスペースは、 **Power BI 管理**  **ポータル**では Premium 容量に、アプリワークスペースの場合は [**ワークスペース**] ウィンドウで割り当てることができます。

容量管理者、および Office 365 全体管理者または Power BI サービス管理者は、 **Power BI 管理**  **ポータル**でワークスペースを一括で割り当てることができます。 一括割り当ては次の対象に適用できます。

- **ユーザー別のワークスペース**:個人用ワークスペースを含む、これらのユーザーが所有するすべてのワークスペースは、Premium 容量に割り当てられます。 これには、既に別の Premium 容量に割り当てられているワークスペースの再割り当てが含まれます。 さらに、ユーザーにはワークスペースの割り当てのアクセス許可も割り当てられます。
...
ワークスペースは、ワークスペースの管理者であり、割り当てアクセス許可を持つユーザーを提供する**ワーク**スペースウィンドウを使用して、Premium 容量に追加できます。

![ワークスペースウィンドウを使用して、ワークスペースを Premium 容量に割り当てる](media/whitepaper-premium-deployment/assign-workspace-capacity.png)

ワークスペース管理者は、割り当てアクセス許可を必要とせずに、容量から (共有容量に) ワークスペースを削除できます。 専用容量からワークスペースを削除すると、ワークスペースが共有容量に効果的に再配置されます。 Premium 容量からワークスペースを削除すると、負の結果が生じる可能性があります。たとえば、共有コンテンツが無料のライセンスユーザー Power BI に使用できなくなった場合や、スケジュールされた更新がサポートされている上限を超過した場合などです。共有容量別。

Power BI サービスでは、Premium 容量に割り当てられたワークスペースは、ワークスペース名をもの菱形アイコンによって簡単に識別できます。

![Premium 容量に割り当てられたワークスペースの特定](media/whitepaper-premium-deployment/premium-diamond-icon.png)

### <a name="monitoring-capacities"></a>容量の監視

Premium 容量を監視すると、管理者は容量のパフォーマンスを把握できます。 容量を監視するには、 [Power BI Premium Capacity Metrics アプリ](service-admin-premium-monitor-capacity.md)または[Power BI 管理者ポータル](service-admin-premium-monitor-portal.md)を使用します。

#### <a name="interpreting-metrics"></a>メトリックの解釈

メトリックを監視して、リソースの使用状況とワークロードアクティビティに関するベースラインの理解を確立する必要があります。 容量が遅い場合は、監視するメトリックと、実行できる結論を把握しておくことが重要です。

クエリは、レポートユーザーに応答性の高いエクスペリエンスを提供し、より高いクエリスループットを実現するために、1秒以内に完了する必要があります。 通常、バックグラウンドプロセス (更新を含む) が完了するまでに時間がかかります。

一般に、低速レポートは過剰な容量を示すことがあります。 レポートの読み込みに失敗した場合、これは、加熱されていない容量を示しています。 どちらの場合も、根本原因は次のような多くの要因に起因する可能性があります。

- **失敗したクエリ**はメモリ不足を示し、モデルをメモリに読み込むことができませんでした。 Power BI サービスは、失敗する前にモデルの読み込みを30秒間試行します。
...
- **クエリの期間が長い**場合は、モデルのデザインが最適化されていないことを示すことがあります。特に、複数のデータセットが容量でアクティブな場合に、1つのデータセットだけが長いクエリ期間を生成している場合です これにより、キャパシティが十分なものであり、問題のあるデータセットが非常に最適化されているか、または低速であることがわかります。 実行時間の長いクエリは、他のプロセスに必要なリソースへのアクセスをブロックできるため、問題になる可能性があります。
...
最大限の使用率とパフォーマンスを実現するために、一般的な推奨事項としてボードで実行できるベストプラクティスがいくつかあります。 不足している機能には次が含まれます。

- 個人用ワークスペースではなくアプリワークスペースを使用する
- ビジネスクリティカルなセルフサービス BI (SSBI) を異なる容量に分割する

  ![ビジネスクリティカルなセルフサービス BI を異なる容量に分割する](media/whitepaper-premium-deployment/separate-capacities.png)
...
多くの理由から、低速の Premium 容量に寄与する可能性があります。 この質問には、低速での意図を理解するために、さらに情報が必要です。 レポートの読み込みに時間がかかる場合は、 読み込みに失敗しているか。 ユーザーがレポートを操作すると、レポートビジュアルの読み込みまたは更新が遅くなるか。 更新の完了に予想以上に長い時間がかかっているか、以前に経験があるか。

その理由を理解していれば、調査を開始できます。 次の6つの質問への回答は、さらに具体的な問題に対処するのに役立ちます。

#### <a name="what-content-is-using-up-my-capacity"></a>容量を使用しているコンテンツ

**Power BI Premium Capacity Metrics**アプリを使用して容量によってフィルター処理したり、ワークスペースのコンテンツのパフォーマンスメトリックを確認したりすることができます。 Premium 容量内に格納されているすべてのコンテンツの過去7日間におけるパフォーマンスメトリックとリソース使用率を1時間ごとに確認することができます。 これは多くの場合、Premium 容量のパフォーマンスに関する一般的な問題をトラブルシューティングするときに最初に行う手順です。
...
さらに、Power BI Premium 容量メトリックアプリでは、アクティブメモリは、過去3分間に使用されているために削除できないレポートに割り当てられたメモリの合計量を示します。 更新待機時間の急激な増加は、大規模なデータセットやアクティブなデータセットと相関させることができます。
...
#### <a name="why-are-reports-slow"></a>レポートの速度が低下するのはなぜですか。

次の表に、考えられる問題とその対処方法を示します。

##### <a name="insufficient-capacity-resources"></a>容量リソースが不足しています

| 考えられる説明 | 識別する方法 | 解決方法 |
| --- | --- | --- |
| アクティブなメモリの合計が増加しています (過去3分間に使用されているため、モデルを削除できません)<br><br> クエリの待機時間の複数のスパイク<br><br> 更新の待機時間の増加による複数のスパイク | メモリメトリック\[ [18](#endnote-18)\]を監視し、削除\[回数を[19](#endnote-19)にする\] | モデルサイズを減らすか、DirectQuery モードに変換します。このセクションの「[モデルの最適化](#optimizing-models)」を参照してください。<br><br> 容量をスケールアップする<br><br> コンテンツを別の容量に割り当てる |

##### <a name="inefficient-report-designs"></a>非効率的なレポートデザイン
...
##### <a name="high-concurrent-report-usage"></a>高同時レポート使用率

| 考えられる説明 | 識別する方法 | 解決方法 |
| --- | --- | --- |
| クエリの待機時間が長い<br><br> CPU の飽和<br><br> DQ/LC 接続の制限を超えました | CPU 使用率\[ [21](#endnote-21)\]、クエリ待機時間、および DQ/LC 使用\[率[22](#endnote-22) \]メトリック + クエリ期間を監視します。変動する場合は、同時実行の問題を示すことができます。 | 容量をスケールアップするか、コンテンツを別の容量に割り当てます。<br><br> 1ページあたりのビジュアル数を減らすことでレポートを再設計する |

#### <a name="why-are-reports-not-loading"></a>レポートが読み込まれていないのはなぜですか。

レポートの読み込みに失敗した場合は最悪のシナリオとなり、容量のメモリが不足していること、および過剰に加熱されていることを確認してください。 これは、読み込まれたすべてのモデルがアクティブにクエリされ、削除できず、更新操作が一時停止または遅延された場合に発生する可能性があります。 Power BI サービスは、データセットの読み込みを30秒間試行します。ユーザーには、すぐに再試行するようにという修正候補が表示されます。

現時点では、レポートの読み込みエラーを監視するためのメトリックはありません。 この問題が発生する可能性を特定するには、システムメモリ、特に最大使用率、および使用率が最も高い時間帯を監視します。 データセットの削除と長いデータセットの更新の平均待機時間は、この問題が発生していることを示唆する可能性があります。

この問題が発生する可能性が非常に高い場合は、優先度の問題とは見なされないことがあります。 レポートのユーザーには、サービスがビジー状態であることと、短時間で再試行する必要があることが通知されます。 これが頻繁に発生する場合は、Premium 容量をスケールアップするか、コンテンツを別の容量に割り当てることで、問題を解決できます。

容量管理者 (および Power BI サービス管理者) は、**クエリ失敗**のメトリックを監視して、いつ発生するかを判断できます。 また、容量を再起動し、システムの過負荷が発生した場合にすべての操作をリセットすることもできます。

#### <a name="why-are-refreshes-not-starting-on-schedule"></a>更新がスケジュールどおりに開始されないのはなぜですか。

スケジュールされた更新の開始時刻は保証されません。 Power BI サービスは、常にバックグラウンド操作で対話操作の優先順位を設定することを思い出してください。 更新は、次の2つの条件が満たされた場合に発生する可能性があるバックグラウンド操作です。

- メモリが不足しています
- Premium 容量に対してサポートされている同時更新の数を超えていません

条件が満たされていない場合、条件が好ましいまで更新がキューに登録されます。

完全な更新を行うには、現在のデータセットのメモリサイズが少なくとも2倍必要であることを思い出してください。 十分なメモリがない場合は、モデルの削除によってメモリが解放されるまで更新を開始できません。これは、1つ以上のデータセットが非アクティブになり、削除できるようになるまで遅延を意味します。

サポートされている最大同時更新数が、バックエンドの v コアの1.5 倍に設定されていることを思い出してください。

スケジュールされた更新は、次回のスケジュールされた更新が開始される前に開始できない場合に失敗します。 UI から手動でトリガーされたオンデマンド更新は、失敗するまで最大3回実行します。

容量管理者 (および Power BI サービス管理者) は、**平均更新待機時間 (分)** メトリックを監視して、スケジュールされた時刻から操作の開始までの平均遅延時間を判断できます。

通常、管理上の優先順位ではありませんが、データ更新時に影響を与える場合は、十分なメモリが使用可能であることを確認してください。 これには、既知の十分なリソースを持つ容量にデータセットを分離することが含まれる場合があります。 また、管理者はデータセットの所有者と連携して、スケジュールされたデータ更新時間をずらしたり減らしたりして、競合を最小限に抑えることができます。 管理者が更新キューを表示したり、データセットのスケジュールを取得したりすることはできないことに注意してください。

#### <a name="why-are-refreshes-slow"></a>更新が遅いのはなぜですか。

更新は、低速であるか、または (前の一般的な質問アドレスとして) 低速である可能性があります。

実際に更新が遅い場合、次のような理由が考えられます。
...
容量管理者 (および Power BI サービス管理者) は、**平均更新期間 (分)** メトリックを監視して、時間の経過と共に比較するためのベンチマークと、平均遅延**時間 (分)** の平均値を確認します。スケジュールされた時刻から操作の開始までの平均遅延時間。

増分更新では、特に大規模なモデルテーブルのデータ更新期間を大幅に短縮できます。 増分更新には、次の4つの利点があります。

- **更新が高速になり**ます。読み込む必要があるのはテーブルのサブセットだけで、CPU とメモリの使用量を減らすことができます。また、複数のパーティションを更新する場合は、並列処理の方が高い場合
- **更新が行われるのは、必要な場合のみ**です。増分更新ポリシーは、データが変更された場合にのみ読み込むように構成できます。
- **更新の信頼性が向上**します。揮発性のデータソースシステムへの接続の実行が短いほど、切断の影響を受けにくくなります。
- **モデルはトリムのまま**です。増分更新ポリシーを構成して、スライディングウィンドウの期間を超えて履歴を自動的に削除することができます。

詳細については、 [Power BI Premium ドキュメントの「増分更新」](service-premium-incremental-refresh.md)を参照してください。

#### <a name="why-are-data-refreshes-not-completing"></a>データ更新が完了していないのはなぜですか。

データ更新が開始されても完了しない場合は、いくつかの理由が考えられます。

- Premium 容量にモデルが1つしかない場合、つまりモデルサイズが非常に大きい場合でも、メモリ不足になります。
- データソースシステムの切断、無効なアクセス許可、またはゲートウェイエラーを含む容量以外の理由

容量管理者 (および Power BI サービス管理者) は、**メモリ不足のメトリックによって更新エラー**を監視できます。
...
効率的でスケーラブルなソリューションを実現するには、最適なモデル設計が不可欠です。 ただし、このホワイトペーパーでは、完全な説明については説明しません。 このセクションでは、モデルを最適化する際に考慮する主な領域について説明します。

#### <a name="optimizing-power-bi-hosted-models"></a>Power BI ホストモデルの最適化

Premium 容量でホストされるモデルの最適化は、データソースとモデルレイヤーで実現できます。

インポートモデルの最適化の可能性について考えてみましょう。

![インポートモデルの最適化の可能性](media/whitepaper-premium-deployment/import-model-optimizations.png)
...
- Power Query のクエリデザインでは、複雑な変換や、特に異なるデータソースをマージする複雑な変換 (データウェアハウスは、抽出、変換、読み込みの段階でこれを実現) を最小化または削除できます。 また、適切なデータソースのプライバシーレベルが設定されていることを確認します。これにより、すべての結果を読み込んでクエリ全体の結果を生成するために Power BI が必要になることを回避できます。
- モデル構造によって、読み込むデータが決定され、モデルのサイズに直接影響します。 列を削除したり、行 (特に履歴データ) を削除したり、集計されたデータを読み込むことによって (詳細データを読み込んだり)、不要なデータが読み込まれないように設計することができます。 非常に効率的に格納または圧縮されない高カーディナリティ列 (特にテキスト列) を削除することにより、サイズを大幅に縮小できます。
- 双方向のフィルター処理を可能にする説得力のある理由がない限り、単一方向のリレーションシップを構成すると、モデルクエリのパフォーマンスを向上させることができます。 また、双方向のフィルター処理ではなく、CROSSFILTER 関数も使用することを検討してください。
- 集計テーブルは、事前に集計されたデータを読み込むことによってクエリの応答速度を向上させることができますが、これにより、モデルのサイズが増加し、更新時間が長くなります。 一般に、集計テーブルは、非常に大規模なモデルまたは複合モデルのデザイン用に予約されている必要があります。
- 計算テーブルと列により、モデルのサイズが増加し、更新時間が長くなります。 一般に、データソースでデータを具体化または計算する場合は、ストレージサイズを小さくして更新時間を短縮することができます。 これが不可能な場合は Power Query カスタム列を使用すると、ストレージの圧縮が向上します。
- メジャーと RLS ルールの DAX 式を調整することができます。たとえば、コストの高い数式を避けるためのロジックを書き直すことがあります。
- 増分更新では、更新時間を大幅に短縮し、メモリと CPU を節約できます。 増分更新は、モデルサイズのトリミングを維持したまま、履歴データを削除するように構成することもできます。
- モデルは、クエリパターンが異なる場合や競合している場合に、2つのモデルとして再設計できます。 たとえば、一部のレポートでは、すべての履歴に対する高レベルの集計が表示され、24時間の待機時間が許容される場合があります。 その他のレポートは、今日のデータに関するものであり、個々のトランザクションに詳細にアクセスする必要があります。 すべてのレポートを満たすために1つのモデルを設計するのではなく、各要件に合わせて最適化された2つのモデルを作成します。

DirectQuery モデルの最適化の可能性を考慮してください。 基になるデータソースへのクエリ要求がモデルによって発行されると、データソースの最適化は、応答性の高いモデルクエリの配信に不可欠です。
...
- 双方向のフィルター処理を可能にする説得力のある理由がない限り、単一方向のリレーションシップを構成すると、モデルクエリのパフォーマンスを向上させることができます。 また、参照整合性が適用されると想定するようにモデルリレーションシップを構成する必要があります (この場合は、外部結合ではなく、より効率的な内部結合を使用してデータソースクエリになります)。
- [カスタム列またはモデルの計算列を作成 Power Query は避けてください]: 可能な場合は、データソースでこれらを具体化します。
- メジャーと RLS ルールの DAX 式を調整することができます。たとえば、コストの高い数式を避けるためのロジックを書き直すことがあります。

複合モデルの最適化の可能性について考えてみましょう。 複合モデルでは、インポートテーブルと DirectQuery テーブルを組み合わせて使用できることを思い出してください。
...
- 通常、バランスの取れた設計を実現するには、(ビジネスエンティティを表す) ディメンション型テーブルをデュアルストレージモードおよびファクトタイプテーブル (多くの場合、運用ファクトを表す大きなテーブル) を DirectQuery ストレージモードとして構成します。 デュアルストレージモードは、インポートと DirectQuery の両方のストレージモードを意味します。これにより、Power BI サービスは、パススルーのネイティブクエリを生成するときに使用する最も効率的なストレージモードを決定できます。
- ゲートウェイに十分なリソースがあること、できれば十分なネットワーク帯域幅とデータソースに近接していることを確認します。
- インポートストレージモードとして構成された集計テーブルでは、DirectQuery ストレージモードのファクト型テーブルを集計するときに、クエリパフォーマンスを大幅に向上させることができます。 この場合、集計テーブルを使用すると、モデルのサイズが増加し、更新時間が長くなります。多くの場合、クエリの高速化には、このようなトレードオフが許容されます。

#### <a name="optimizing-externally-hosted-models"></a>外部でホストされるモデルの最適化

「 [Power BI ホスト型モデルの最適化](#optimizing-power-bi-hosted-models)」で説明されている多くの最適化の可能性は、Azure Analysis Services と SQL Server Analysis Services で開発されたモデルにも適用されます。 [例外のクリア] は、複合モデルや集計テーブルなど、現在サポートされていない特定の機能です。

外部でホストされるデータセットに関する追加の考慮事項は、Power BI サービスに関連するデータベースをホストすることです。 Azure Analysis Services の場合、これは、Power BI テナント (ホームリージョン) と同じリージョンに Azure リソースを作成することを意味します。 SQL Server Analysis Services では、IaaS の場合、これは VM を同じリージョンでホストすることを意味し、オンプレミスの場合は、効率的なゲートウェイのセットアップを保証することを意味します。

ただし、Azure Analysis Services データベースと SQL Server Analysis Services 表形式データベースでは、モデルが完全にメモリに読み込まれ、クエリをサポートするために常に保持されている必要があることに注意してください。 Power BI サービスと同様に、更新中にモデルをオンラインのままにする必要がある場合は、更新に十分なメモリが必要です。 Power BI サービスとは異なり、使用法に従って、モデルがメモリに自動的に期限切れになったり、メモリから除外されたりするという概念はありません。 したがって、Power BI Premium では、メモリ使用量が少ないモデルクエリを最大限に活用するための効率的なアプローチが提供されます。

### <a name="capacity-planning"></a>容量計画

Premium 容量のサイズによって、使用可能なメモリとプロセッサのリソースと容量に課される制限が決まります。 また、Premium 容量を複数作成することは、ワークロードを相互に分離するのに役立ちます。 記憶域は容量ノードあたり 100 TB であることに注意してください。これは、すべてのワークロードにとって十分ではない可能性があります。

特に作成する初期容量では、Premium 容量のサイズと数を決定することは困難な場合があります。 容量のサイズ変更を行うには、最初の手順として、予定されている毎日の使用状況を表す平均ワークロードを把握します。 すべてのワークロードが等しいとは限りません。 たとえば、100スペクトラムの一方の端では、1つのビジュアルを含む1つのレポートページに同時にアクセスするユーザーが、簡単に実現できます。 ただし、スペクトルのもう一方の端にある-100 異なるレポートに100アクセスしているユーザーが、レポートページ上の100ビジュアルを使用している場合は、それらのレポートが、容量リソースに対して大幅に異なる要求を実行します。

そのため、容量管理者は、環境、コンテンツ、予想される使用状況に固有の多くの要因を考慮する必要があります。 上書き目標は、一貫したクエリ時間、許容される待機時間、および削除率を提供すると同時に、容量の使用率を最大化することです。 考慮すべき要因は次のとおりです。

- **モデルサイズとデータ特性**:クエリまたは更新を可能にするには、インポートモデルをメモリに完全に読み込む必要があります。 LC/DQ データセットでは、プロセッサ時間が大幅に長く、場合によっては複雑なメジャーや RLS ルールを評価するのに大きなメモリが必要になります。 メモリとプロセッサのサイズ、および LC/DQ クエリのスループットは容量のサイズによって制限されます。
- **同時アクティブなモデル**:さまざまなインポートモデルの同時クエリを実行すると、メモリに保持されている場合の応答性とパフォーマンスが最適になります。 大量のクエリを実行するすべてのモデルをホストするのに十分なメモリが必要であり、更新を可能にするためのメモリが追加されている必要があります。
- **インポートモデルの更新**:更新の種類 (完全または増分)、Power Query クエリと計算テーブル/列ロジックの実行時間と複雑さは、メモリや特にプロセッサ使用率に影響を与える可能性があります。 同時更新は容量サイズによって制限されます (1.5 x バックエンド v コア、切り上げ)。
- **同時実行クエリ**:多くの同時実行クエリは、プロセッサまたは LC/DQ 接続が容量制限を超えた場合に、応答しないレポートを生成することがあります。 これは特に、多くのビジュアルを含むレポートページの場合に当てはまります。
- **データフロー、ページ分割されたレポート、AI 関数**:容量は、データフロー、ページ分割されたレポート、AI 関数をサポートするように構成できます。各機能には、容量のメモリの構成可能な最大パーセンテージが必要です。 メモリはデータフローに動的に割り当てられますが、ページ分割されたレポートと AI ワークロードに静的に割り当てられます。

これらの要因に加えて、容量管理者は複数の容量を作成することもできます。 複数の容量を使用すると、ワークロードを分離でき、優先順位の高いワークロードで確実にリソースを確保するように構成できます。 たとえば、ビジネスクリティカルなワークロードをセルフサービス BI (SSBI) ワークロードから分離するために、2つの容量を作成できます。 ビジネスクリティカルな容量を使用して、会社の大規模なモデルを分離し、リソースを保証し、作成アクセス権を IT 部門にのみ付与することができます。 SSBI 容量を使用して、より小さなモデルをホストし、ビジネスアナリストにアクセス権を付与することができます。 SSBI の容量は、クエリや更新の待機が許容できる時間内に発生する可能性があります。

時間の経過と共に、容量管理者は、ワークスペース間でのコンテンツの移動、容量のスケールアップまたはスケールダウンによって、容量全体のワークスペースのバランスを取ることができます。 一般に、大規模なモデルをホストするには、スケールアップして、同時実行性を向上させる必要があります。

ライセンスを購入すると、テナントには v コアが提供されます。 **P3**サブスクリプションの購入を使用して、1つまたは最大4つの Premium 容量 (1 x P3、2 x P2、または 4 x P1) を作成できます。 また、P2 容量を P3 容量にアップサイズする前に、v コアを分割して2つの P1 容量を作成することを考慮する必要があります。

### <a name="testing-approaches"></a>テストアプローチ

容量のサイズを決定したら、制御された環境を作成してテストを実行できます。 実際には、Azure (Sku) 容量を作成することをお勧めします。これは、P1 容量が A4 容量と同じサイズであることを示しています。 P2 および P3 容量は、それぞれ A5 および A6 容量と同じサイズです。 Azure 容量は迅速に作成でき、時間単位で課金されます。 そのため、テストが完了したら、簡単に削除してコストを抑えることができます。

テストコンテンツは、Azure 容量で作成されたワークスペースに追加できます。また、1人のユーザーがレポートを実行して、クエリの現実的で代表的なワークロードを生成することができます。 インポートモデルがある場合は、各モデルの更新も実行する必要があります。 その後、監視ツールを使用してすべてのメトリックを確認し、リソース使用率を把握できます。

テストは反復可能であることが重要です。テストは複数回実行する必要があり、そのたびにほぼ同じ結果を提供する必要があります。 これらの結果の平均値を使用して、実際の運用条件に基づいてワークロードを推定し、推定することができます。

ロードテストを行うために必要な容量とレポートが既にある場合は、 [PowerShell のロード生成ツール](https://aka.ms/PowerBILoadTestingTool)を使用して、ロードテストをすばやく生成します。 このツールを使用すると、容量を1時間に実行できる各レポートのインスタンスの数を見積もることができます。 このツールを使用すると、個々のレポートの表示、または複数の異なるレポートを並行して表示するためにキャパシティの能力を評価できます。 詳細については、Microsoft [Power BI ビデオを参照してください。Premium 容量](https://www.youtube.com/watch?time_continue=1860&v=C6vk6wk9dcw)。

より複雑なテストを生成するには、現実的なワークロードをシミュレートするロードテストアプリケーションを開発することを検討してください。 詳細については、「 [Visual Studio のロードテストを使用したアプリケーション Power BI のロードテスト](https://blogs.msdn.microsoft.com/charles_sterling/2018/04/04/webinar-load-testing-power-bi-applications-with-visual-studio-load-test/)」を参照してください。
...
- **クエリ期間**では、管理者によってデータセット (クエリされた上位のデータセットから開始) によってフィルター処理されたデータセットが表示されます。
...
容量がワークロードを完了するのに十分なメモリがあるかどうかを判断するために、Power BI 管理者はアプリの [**データセット**] タブで [使用済み**メモリの割合**] ビジュアルを参照できます。 **すべて**(合計) メモリは、データセットがアクティブにクエリまたは処理されているかどうかに関係なく、メモリに読み込まれたデータセットによって消費されるメモリを表します。 **アクティブな**メモリは、アクティブに処理されているデータセットによって消費されるメモリを表します。
...
一般に、容量の平均 CPU 使用率は 80% 未満にする必要があります。 この値を超えると、容量が CPU 飽和状態に近づいていることを意味します。
...
同様のパターンは、CPU の飽和に寄与する場合にバックグラウンド操作で検出されることがあります。 Power BI 管理者は、特定のデータセットの更新時間の急激なスパイクを探すことができます。これは、(おそらく、他の実行中のデータセットの更新や対話型のクエリなどが原因で) CPU の飽和状態を示すことができます。 このインスタンスでは、アプリの**システム**ビューを参照しても、CPU が 100% であることがわかるとは限りません。 **システム**ビューには1時間ごとの平均が表示されますが、大量の操作に対して CPU が飽和状態になることがあります。これは、待機時間のスパイクとして表示されます。
...
<a name="endnote-01"></a>\[1\]このテクニカルペーパーは、Power BI クラウドサービスでのみサポートされている Power BI Premium を懸念しています。そのため、Power BI Report Server のインストールに必要なライセンスが一部に含まれていることを除き Power BI Report Server はスコープに含まれていません。Sku を Power BI Premium します。

<a name="endnote-02"></a>\[2\]アプリケーションユーザーの代わりにコンテンツを埋め込むために使用するクラウドサービスとして Power BI は、サービスとしてのプラットフォーム (PaaS) です。 この種類の埋め込みは、異なる2つの製品で実現できます。そのうちの1つは Power BI Premium です。

<a name="endnote-03"></a>\[3\]プッシュ、ストリーミング、ハイブリッドのデータセットは premium 容量には格納されないため、premium 容量をデプロイ、管理、および監視する場合は考慮されません。

<a name="endnote-04"></a>\[4\] Power BI のコンテンツタイプとしての Excel ブックは、premium 容量には格納されないため、premium 容量をデプロイ、管理、または監視する場合には考慮されません。

<a name="endnote-05"></a>\[5\]つのビジュアルは、スライサーの相互作用を無視するように構成できます。 詳細については、 [Power BI レポートでの視覚化の相互作用](service-reports-visual-interactions.md)に関するドキュメントを参照してください。

<a name="endnote-06"></a>\[6\]ファイルに対してを使用して、Power BI Desktop ファイルのサイズとタスクマネージャーのメモリを比較することにより、サイズの違いを判断できます。

<a name="endnote-07"></a>\[7\] Microsoft データソースのサポートには、SQL Server、Azure データブリック、Azure HDInsight Spark (ベータ)、Azure SQL Database、および Azure SQL Data Warehouse が含まれます。 その他のソースの詳細については、 [Power BI ドキュメントの「Direct Query でサポートされるデータソース](desktop-directquery-data-sources.md)」を参照してください。

<a name="endnote-08"></a>\[8\] Power BI Premium は、最大 10 GB のサイズの Power BI Desktop (.pbix) ファイルのアップロードをサポートしています。 アップロードされたデータセットのサイズは、更新の結果として最大 12 GB まで拡張できます。 アップロードの最大サイズは SKU によって異なります。 詳細については、「[大規模なデータセットの Power BI Premium サポート](service-premium-large-datasets.md)」ドキュメントを参照してください。
...
<a name="endnote-11"></a>\[11\]これらのタイミングは、いつでも変更される可能性があります。

<a name="endnote-12"></a>\[12\]これは、現在プレビューの段階にあるマルチ geo と呼ばれます。 複数の geo デプロイの原理は、通常、パフォーマンスやスケールではなく、企業または政府のコンプライアンスのためのものです。 レポートとダッシュボードの読み込みでも、メタデータのホームリージョンへの要求が含まれます。 詳細については、 [Power BI Premium (プレビュー) の複数の Geo サポート](service-admin-premium-multi-geo.md)に関するドキュメントを参照してください。

<a name="endnote-13"></a>\[13\]ユーザーは、ジョブを使用して Power BI サービスをオーバーロードしたり、過度に複雑なクエリを記述したり、循環参照を作成したりすることによって、パフォーマンスの問題を引き起こす可能性があります。

<a name="endnote-14"></a>\[14\]組織のワークスペース全体を割り当てるオプションは推奨されていません。対象となるアプローチの方が適しています。 一般に、運用環境のコンテンツには個人用ワークスペースを使用しないことをお勧めします。
...
<a name="endnote-16"></a>\[16\]回の更新では、起動するのに十分な CPU またはメモリがない場合に待機できます。

<a name="endnote-17"></a>\[17\]メモリ内のデータセットのサイズがディスク上のサイズよりも最大 20% 大きくなることがあります。

<a name="endnote-18"></a>\[18\]個の平均メモリ使用量 (gb)、最大メモリ消費量 (gb)

<a name="endnote-19"></a>\[19\]データセットの削除

<a name="endnote-20"></a>\[20\]個のデータセットクエリ、データセットの平均クエリ実行時間 (ミリ秒)、データセットの待機回数、データセットの平均待機時間 (ミリ秒)

<a name="endnote-21"></a>\[21\] cpu の高使用率のカウントと、使用率が最も高い cpu 時間 (過去7日間)

<a name="endnote-22"></a>\[22\] DQ/lc 高使用率のカウントと DQ/lc の使用率が最も高い (過去7日間)