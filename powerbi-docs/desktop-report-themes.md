...
Power BI Desktop で直接カスタマイズを行うか、またはテーマの JSON ファイルを使用して、 **[書式]** ウィンドウに表示されるほぼすべての要素をカスタマイズし、標準化することもできます。 この目標は、レポートの外観を細かなレベルまで完全に制御できるようにすることです。

## <a name="how-report-themes-work"></a>レポートのテーマのしくみ
Power BI Desktop レポートにレポート テーマを適用するには、使用可能な組み込みのレポート テーマから選択するか、カスタムのテーマを作成またはインポートします。
...
Power BI Desktop でテーマをカスタマイズするには、2 つの方法があります。 それぞれについて順に説明します。


## <a name="customize-report-themes-preview"></a>レポート テーマをカスタマイズする (プレビュー)

**Power BI Desktop** の 2019 年 12 月リリースより、レポート テーマをカスタマイズする方法は 2 つ用意されています。

* Power BI Desktop でテーマを作成およびカスタマイズする (プレビュー)
* カスタムのレポート テーマの JSON ファイルを作成およびカスタマイズする

Power BI Desktop でテーマを直接カスタマイズするには、まず **[ファイル] > [オプションと設定] > [オプション]** を選択してから、次の図に示すように、 **[プレビュー機能]** セクションで **[現在のテーマのカスタマイズ]** の横のボックスをオンにします。

![カスタマイズされたテーマを有効にする](media/desktop-report-themes/report-themes_5a.png)

プレビュー機能を有効にするために Power BI Desktop の再起動が必要になる場合があります。

再起動後、 **[ホーム]** リボンを選択し、リボンから **[テーマの切り替え] > [現在のテーマのカスタマイズ]** を選択して、現在のテーマのカスタマイズを開始できます。 ダイアログが表示され、既存のテーマをカスタマイズするためのさまざまな方法が表示されます。

![テーマをカスタマイズする](media/desktop-report-themes/report-themes_5b.png)

既存のテーマを使用していくつかの調整を行う場合は、次の図に示すように、既存のテーマを選択し、ダイアログから **[現在のテーマのカスタマイズ]** を選択します。 

![現在のテーマをカスタマイズする](media/desktop-report-themes/report-themes_5c.png)

> [!NOTE]
> 前の画像は、新しいリボンが有効になっている状態でキャプチャされたもので、現在プレビューの段階です。 新しいリボンのプレビューを有効にするには、 **[ファイル] > [オプションと設定] > [オプション]** を選択し、 **[プレビュー機能]** セクションで **[New ribbon preview]\(新しいリボンのプレビュー\)** を選択します。

カスタマイズできるテーマの設定は次のカテゴリにあり、[テーマのカスタマイズ] ダイアログに反映されています。

* テーマ名 (カスタマイズするテーマに名前を指定します) とさまざまな色設定 (テーマの色、センチメントの色、分岐の色など)
* テキスト設定 (フォント ファミリ、サイズ、色のほか、軸のタイトル、色、カード、KPI、およびタブ ヘッダーなど)
* 背景、境界線、ヘッダー、ツールヒントなどのビジュアル要素
* ページ要素 (壁紙や背景など)
* フィルター ペインの設定 (背景色、透明度、フォントとアイコンの色、サイズ、フィルター カードなど)

変更を加えて **[適用して保存]** ボタンを選択すると、テーマが保存され、現在のレポートで使用できるようになり、エクスポートできるようになります。 

この方法で現在のテーマをカスタマイズすると、テーマのカスタマイズのビジュアル作業をすばやく簡単に行うことができます。 ただし、次のセクションで説明するように、テーマへの限られた調整の中には、テーマの JSON ファイルの変更が必要なものがいくつかあります。

> [!TIP]
> **[現在のテーマのカスタマイズ]** ダイアログで、ビジュアル要素を使用してほとんどのテーマ要素をカスタマイズできます。次に、JSON ファイルをエクスポートし、(JSON ファイル自体を変更することによって) 手動で細かく調整を行うことができます。 その後、細かく調整した JSON ファイルの名前を変更し、インポートして、必要なすべての調整を行うことができます。
